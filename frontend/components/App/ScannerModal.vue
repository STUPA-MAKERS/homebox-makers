                ...
                >
                    <div class="flex">
                        ...
                        <span class="flex-1 text-center text-sm font-medium">
                            {{ detectedBarcodeType }}
                            {{ $t("scanner.barcode_detected_message") }}:
                ...
                <div class="mt-4">
                    <Select v-model="selectedSource">
                        <SelectTrigger class="w-full">
                            <SelectValue
                                :placeholder="t('scanner.select_video_source')"
                            />
                        </SelectTrigger>
                        <SelectContent>
                            <SelectItem
...
const open = computed(
    () => activeDialog && activeDialog.value === DialogID.Scanner,
);
...
const handleButtonClick = () => {
    openDialog(DialogID.ProductImport, {
        params: { barcode: detectedBarcode.value },
    });
};
                    ...
                    try {
                        ...
                        switch (bcfmt) {
                            case BarcodeFormat.QR_CODE: {
                                ...
                                const sanitizedId = text.replace(
                                    /[^a-zA-Z0-9-_/]/g,
                                    "",
                                );
                                closeDialog(DialogID.Scanner);
                                ...
                                if (sanitizedId.startsWith("/")) {
                                    navigateTo(sanitizedId);
                                } else {
                                    navigateTo(`/a/${sanitizedId}`);
                                }
                                return;
                            }
                            ...
                            case BarcodeFormat.UPC_EAN_EXTENSION:
                                ...
                                detectedBarcodeType.value = BarcodeFormat[
                                    bcfmt
                                ].replaceAll("_", "-");
                                break;
